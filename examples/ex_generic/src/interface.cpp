...
void create_parameters(){
    ...
    embui.var_create(FPSTR(T_HOSTNAME), "");                // device hostname (autogenerated on first-run)
    embui.var_create(FPSTR(T_APONLY),  FPSTR(T_FALSE));     // режим AP-only (только точка доступа), не трогать
    embui.var_create(FPSTR(T_APPWD), "");                   // пароль внутренней точки доступа

    // параметры подключения к MQTT
    embui.var_create(FPSTR(T_MHOST), "");                   // MQTT server hostname
    embui.var_create(FPSTR(T_MPORT), F("1883"));            // MQTT port

    embui.var_create(FPSTR(T_MUSER), "");                   // MQTT login
    embui.var_create(FPSTR(T_MPASS), "");                   // MQTT pass

    embui.var_create(FPSTR(T_MPREF), embui.mc);             // MQTT topic == use ESP MAC address
    embui.var_create(FPSTR(T_MPERIOD), F("30"));            // интервал отправки данных по MQTT в секундах (параметр в энергонезависимой памяти)
    embui.var_create(FPSTR(F("mqtt_int")), F("30"));            // интервал отправки данных по MQTT в секундах (параметр в энергонезависимой памяти)

    // date/time related vars
    embui.var_create(FPSTR(T_TZSET), "");                   // TimeZone/DST rule (empty value == GMT/no DST)
    embui.var_create(FPSTR(T_USERNTP), "");                 // Backup NTP server
...
void section_main_frame(Interface *interf, JsonObject *data){
    ...
    if(!embui.wifi_sta){                // если контроллер не подключен к внешней AP, открываем вкладку настройки WiFi 
...
void block_settings_netw(Interface *interf, JsonObject *data){
    ...
    interf->text(FPSTR(T_HOSTNAME), FPSTR(T_DICT[lang][TD::D_Hostname]));
...
//    interf->text(FPSTR(T_WCSSID), "", FPSTR(T_DICT[lang][TD::D_WiFiSSID]), false);
    ...
    interf->checkbox(FPSTR(T_APONLY), FPSTR(T_DICT[lang][TD::D_APOnlyMode]));
    interf->password(FPSTR(T_APPWD),  FPSTR(T_DICT[lang][TD::D_MSG_APProtect]));
    ...
    interf->text(FPSTR(T_MHOST), FPSTR(T_DICT[lang][TD::D_MQTT_Host]));
    interf->number(FPSTR(T_MPORT), FPSTR(T_DICT[lang][TD::D_MQTT_Port]));
    interf->text(FPSTR(T_MUSER), FPSTR(T_DICT[lang][TD::D_User]));
    interf->text(FPSTR(T_MPASS), FPSTR(T_DICT[lang][TD::D_Password]));
    interf->text(FPSTR(T_MPREF), FPSTR(T_DICT[lang][TD::D_MQTT_Topic]));
...
void block_settings_time(Interface *interf, JsonObject *data){
    ...
    interf->text(FPSTR(T_TZSET), FPSTR(T_DICT[lang][TD::D_MSG_TZONE]));
    interf->text(FPSTR(T_USERNTP), FPSTR(T_DICT[lang][TD::D_NTP_Secondary]));
...
void set_settings_wifi(Interface *interf, JsonObject *data){
    ...
    SETPARAM(FPSTR(T_HOSTNAME));        // сохраняем hostname в конфиг
...
void set_settings_wifiAP(Interface *interf, JsonObject *data){
    ...
    SETPARAM(FPSTR(T_HOSTNAME));    // эти переменные будут сохранены в конфиг-файл
    SETPARAM(FPSTR(T_APONLY));
    SETPARAM(FPSTR(T_APPWD));
...
void set_settings_mqtt(Interface *interf, JsonObject *data){
    if (!data) return;
    SETPARAM(FPSTR(T_MHOST), strncpy(embui.m_host, embui.param(FPSTR(T_MHOST)).c_str(), sizeof(embui.m_host)-1));
    SETPARAM(FPSTR(T_MUSER), strncpy(embui.m_user, embui.param(FPSTR(T_MUSER)).c_str(), sizeof(embui.m_user)-1));
    SETPARAM(FPSTR(T_MPASS), strncpy(embui.m_pass, embui.param(FPSTR(T_MPASS)).c_str(), sizeof(embui.m_pass)-1));
    SETPARAM(FPSTR(T_MPREF), strncpy(embui.m_pref, embui.param(FPSTR(T_MPREF)).c_str(), sizeof(embui.m_pref)-1));
    SETPARAM(FPSTR(T_MPORT), embui.m_port = embui.param(FPSTR(T_MPORT)).toInt());
...
void set_settings_time(Interface *interf, JsonObject *data){
    ...
    SETPARAM(FPSTR(T_TZSET), embui.timeProcessor.tzsetup((*data)[FPSTR(T_TZSET)]));
    SETPARAM(FPSTR(T_USERNTP), embui.timeProcessor.setcustomntp((*data)[FPSTR(T_USERNTP)]));
